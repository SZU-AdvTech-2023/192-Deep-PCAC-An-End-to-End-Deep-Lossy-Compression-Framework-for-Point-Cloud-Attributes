...
def compress(x_coori,x_color,model, ckpt_dir, latent_points):
    ...
    print('===== Compress =====')
    # load model.
    AttributeEncoder = model.AttributeEncoder()
    AttributeDecoder = model.AttributeDecoder()
    ...
    def loop_attribute_encoder(y):
        y = tf.expand_dims(y, 0)
        z = AttributeEncoder(y)
        return tf.squeeze(z,axis=0)
    y = tf.expand_dims(ys, 0)
    zs = tf.map_fn(loop_attribute_encoder, y, dtype=tf.float32, parallel_iterations=1, back_prop=False)
    ...
    hyper_codes = tf.map_fn(loop_hyper_encoder, zs, dtype=tf.float32, parallel_iterations=1, back_prop=False)
    print("Hyper Encoder")

    z_hats, _ = entropy_bottleneck(hyper_codes, False)
...
def psnr(original, reconstructed):
    # Assuming original and reconstructed are numpy arrays of shape (num_points, 3) for 3D point clouds
    mse = np.mean(np.sum((original - reconstructed)**2, axis=-1))  # Mean Squared Error
...
if __name__ == "__main__":
    ...
    loss_fn = tf.keras.losses.YourLoss()  # Replace with your actual loss function
    ...
    for epoch in range(epochs):
        ...
        with tf.GradientTape() as tape:
            ...
            ori_coori = point_set_ori[:,:,0:3]
            rec_point_cloud = np.concatenate((ori_coori,rec_color),-1)
            pc_rec = postprocess(args.output, rec_point_cloud, int(num_less_than_2048_d),points_num=2048)

            loss = psnr(point_set, pc_rec)
